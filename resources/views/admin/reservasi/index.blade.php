...
<div style="margin-bottom:20px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
  <div>
    <button type="button" class="btn" data-bs-toggle="modal" data-bs-target="#tambahMejaModal"
      style="background:#6F4E37; color:white; border:none; padding:10px 20px; border-radius:8px; cursor:pointer; font-size:14px; display:flex; align-items:center; gap:6px; box-shadow:0 2px 5px rgba(0,0,0,0.2); transition:all 0.2s ease;"
      onmouseover="this.style.background='#5B3E2E'" onmouseout="this.style.background='#6F4E37'">
      <i class="fas fa-plus"></i> Tambah Meja
    </button>
  </div>

  <div>
    <input type="text" id="searchInput" placeholder="🔍 Cari nama reservasi..."
      style="padding:10px 12px; border:1px solid #c4bebe; border-radius:8px; width:260px; font-size:14px; box-shadow:0 2px 5px rgba(0,0,0,0.1); transition:all 0.25s ease; outline:none;"
        ...
        <tr style="text-align:center; border-bottom:1px solid #ddd;">
          ...
          <td>
            <div style="display:flex; justify-content:center; gap:8px; flex-wrap:wrap;">
              <!-- Lihat -->
              <button class="btn-lihat" style="background:#2196F3; color:white; border:none; padding:6px 12px; border-radius:6px; cursor:pointer; font-size:13px; display:flex; align-items:center; gap:5px;"
                ...
                data-catatan="{{ $r->catatan ?? '-' }}" data-bs-toggle="modal" data-bs-target="#lihatReservasiModal">
                <i class="fas fa-eye"></i> Lihat
              </button>

              <!-- Edit -->
              <button class="btn-edit" style="background:#4CAF50; color:white; border:none; padding:6px 12px; border-radius:6px; cursor:pointer; font-size:13px; display:flex; align-items:center; gap:5px;"
                data-id="{{ $r->id }}" data-nama="{{ $r->nama }}" data-jumlah="{{ $r->jumlah_orang }}"
                data-meja="{{ $meja ? $meja->id : '' }}" data-tanggal="{{ $r->tanggal }}" data-jam="{{ $r->jam }}"
                data-catatan="{{ $r->catatan ?? '' }}" data-bs-toggle="modal" data-bs-target="#editReservasiModal">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Konfirmasi Hapus (submit form biasa, tidak pakai AJAX)
    ...
    btn.addEventListener('click', e => {
      ...
      Swal.fire({
        ...
        if(result.isConfirmed){
          form.submit(); // submit biasa -> Laravel redirect
        }
      });
    });
  });
